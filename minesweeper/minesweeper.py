    ...
    def add_knowledge(self, cell, count):
        ...
        for sentence in self.knowledge:
            if sentence.known_mines() is not None:
                mine = sentence.known_mines().copy()
                for c in mine:
                    self.mark_mine(c)
        ...
        for sentence in self.knowledge:
            ...
            for sentence2 in self.knowledge:
                ...
                if sentence.cells.issubset(sentence2.cells):
                    new_cells = list(sentence.cells.symmetric_difference(sentence2.cells))
                    new_count = sentence2.count - sentence.count
                    if len(new_cells) != 0:
                        infer.append(Sentence(new_cells, new_count))
        self.knowledge.extend(infer)
    ...
    def make_random_move(self):
        ...
        if self.make_safe_move() is None:
            for i in range(self.height):
                for j in range(self.width):
                    move = (i, j)
                    if move in self.mines or move in self.moves_made:
                        continue
                    return move
        return None
